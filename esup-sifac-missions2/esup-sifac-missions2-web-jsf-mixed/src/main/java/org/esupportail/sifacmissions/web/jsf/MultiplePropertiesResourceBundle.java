...
public class MultiplePropertiesResourceBundle extends ResourceBundle {
	 ...
	 * Path for bundles.
	 */
	private static final String RESOURCEPATH = "properties/i18n/bundles/";

	/**
	 * Path for bundles order.
	 */
	private static final String SPRINGRESOURCEPATH = "properties/i18n/";
	 ...
	 * @param language
	 */
	public MultiplePropertiesResourceBundle(String language) {
	...
	private List<String> findBaseNames(final String baseName) {
		boolean isLoggable = logger.isDebugEnabled();
		...
		try {
			...
			if (isLoggable) {
				logger.debug("Looking for files named '" + resourceName + "'");
			}
			...
			while (names.hasMoreElements()) {
				...
				if (isLoggable) {
					logger.debug("inspecting: " + jarUrl);
				}
			...
			logger.debug("Number of bundles: " + nbNames);
		} catch (Exception e) {
			e.printStackTrace();
		}
		if (isLoggable) {
			logger.debug("Combine ResourceBundles named: " + bundleNames);
		}
		return bundleNames;
	}
